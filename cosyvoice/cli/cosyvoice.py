...
def _modelscope_snapshot_download(model_id):
    """Load ModelScope only when a non-local upstream model ID is requested."""
...
class CosyVoice:
    ...
    def __init__(self, model_dir, load_jit=False, load_trt=False, fp16=False, trt_concurrent=1):
        ...
        if not os.path.exists(model_dir):
            model_dir = _modelscope_snapshot_download(model_dir)
    ...
    def inference_sft(self, tts_text, spk_id, stream=False, speed=1.0, text_frontend=True, **generation_kwargs):
        for i in tqdm(self.frontend.text_normalize(tts_text, split=True, text_frontend=text_frontend)):
            model_input = self.frontend.frontend_sft(i, spk_id)
            start_time = time.time()
            logging.info('synthesis text {}'.format(i))
            for model_output in self.model.tts(**model_input, stream=stream, speed=speed, **generation_kwargs):
                speech_len = model_output['tts_speech'].shape[1] / self.sample_rate
                logging.info('yield speech len {}, rtf {}'.format(speech_len, (time.time() - start_time) / speech_len))
                yield model_output
                start_time = time.time()

    def inference_zero_shot(self, tts_text, prompt_text, prompt_wav, zero_shot_spk_id='', stream=False, speed=1.0, text_frontend=True, language='Auto', **generation_kwargs):
        prompt_text = self.frontend.text_normalize(prompt_text, split=False, text_frontend=text_frontend, language=language)
        for i in tqdm(self.frontend.text_normalize(tts_text, split=True, text_frontend=text_frontend, language=language)):
    ...
    def inference_cross_lingual(self, tts_text, prompt_wav, zero_shot_spk_id='', stream=False, speed=1.0, text_frontend=True, language='Auto', **generation_kwargs):
        for i in tqdm(self.frontend.text_normalize(tts_text, split=True, text_frontend=text_frontend, language=language)):
    ...
    def inference_instruct(self, tts_text, spk_id, instruct_text, stream=False, speed=1.0, text_frontend=True, **generation_kwargs):
...
class CosyVoice2(CosyVoice):
    ...
    def inference_instruct2(self, tts_text, instruct_text, prompt_wav, zero_shot_spk_id='', stream=False, speed=1.0, text_frontend=True, language='Auto', **generation_kwargs):
        for i in tqdm(self.frontend.text_normalize(tts_text, split=True, text_frontend=text_frontend, language=language)):
    ...
    def __init__(self, model_dir, load_trt=False, load_vllm=False, fp16=False, trt_concurrent=1):
        ...
        if load_trt:
            if self.fp16 is True:
                logging.info('DiT TensorRT FP16 engine selected; NVIDIA marks this engine as experimental for performance.')
            self.model.load_trt('{}/flow.decoder.estimator.{}.mygpu.plan'.format(model_dir, 'fp16' if self.fp16 is True else 'fp32'),
                                '{}/flow.decoder.estimator.fp32.onnx'.format(model_dir),
                                trt_concurrent,
                                self.fp16)
...
def AutoModel(**kwargs):
    if not os.path.exists(kwargs['model_dir']):
        kwargs['model_dir'] = _modelscope_snapshot_download(kwargs['model_dir'])
