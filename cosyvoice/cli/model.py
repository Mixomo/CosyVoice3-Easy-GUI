...
class CosyVoiceModel:
    ...
    def load_trt(self, flow_decoder_estimator_model, flow_decoder_onnx_model, trt_concurrent, fp16):
        ...
        with open(flow_decoder_estimator_model, 'rb') as f:
            # TensorRT's INFO logger emits a large block of engine/stream
            # diagnostics for every load.  Keep actual TensorRT errors visible
            # while leaving normal inference logs readable in the GUI console.
    ...
    def llm_job(self, text, prompt_text, llm_prompt_speech_token, llm_embedding, uuid, generation_config=None):
...
class CosyVoice2Model(CosyVoiceModel):
    ...
    def tts(self, text=torch.zeros(1, 0, dtype=torch.int32), flow_embedding=torch.zeros(0, 192), llm_embedding=torch.zeros(0, 192),
        ...
        if source_speech_token.shape[1] == 0:
            llm_generation = {
                "sampling": int(kwargs.get("top_k", 25)),
                "top_k": int(kwargs.get("top_k", 25)),
                "top_p": float(kwargs.get("top_p", 0.8)),
                "temperature": float(kwargs.get("temperature", 1.0)),
                "ras_window": int(kwargs.get("ras_window", 10)),
                "ras_repetition_threshold": float(kwargs.get("ras_repetition_threshold", 0.1)),
                "min_token_text_ratio": float(kwargs.get("min_token_text_ratio", 2.0)),
                "max_token_text_ratio": float(kwargs.get("max_token_text_ratio", 20.0)),
            }
            p = threading.Thread(target=self.llm_job, args=(text, prompt_text, llm_prompt_speech_token, llm_embedding, this_uuid, llm_generation))
        else:
            p = threading.Thread(target=self.vc_job, args=(source_speech_token, this_uuid))
        p.start()
        flow_generation = {}
        if self.__class__.__name__ == 'CosyVoice3Model':
            flow_generation = {
                "flow_steps": int(kwargs.get("flow_steps", 10)),
                "flow_temperature": float(kwargs.get("flow_temperature", 1.0)),
            }
        if stream is True:
            token_offset = 0
...
class CosyVoice3Model(CosyVoice2Model):
    ...
    def token2wav(self, token, prompt_token, prompt_feat, embedding, token_offset, uuid, stream=False, finalize=False, speed=1.0,
                  flow_steps=10, flow_temperature=1.0):
        with torch.amp.autocast('cuda', enabled=self.fp16):
            tts_mel, _ = self.flow.inference(token=token.to(self.device, dtype=torch.int32),
                                             token_len=torch.tensor([token.shape[1]], dtype=torch.int32).to(self.device),
                                             prompt_token=prompt_token.to(self.device),
                                             ...
                                             prompt_feat_len=torch.tensor([prompt_feat.shape[1]], dtype=torch.int32).to(self.device),
                                             embedding=embedding.to(self.device),
                                             n_timesteps=int(flow_steps),
                                             temperature=float(flow_temperature),
                                             streaming=stream,
                                             finalize=finalize)
            tts_mel = tts_mel[:, :, token_offset * self.flow.token_mel_ratio:]
            # append mel cache
