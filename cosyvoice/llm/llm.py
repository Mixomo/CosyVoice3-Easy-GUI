    ...
    def sampling_ids(
        ...
        try:
            top_ids = self.sampling(
                ...
                win_size=int(ras_window), tau_r=float(ras_repetition_threshold),
            )
        except TypeError:
...
class Qwen2Encoder(torch.nn.Module):
    def __init__(self, pretrain_path):
        super().__init__()
        # Keep the backend order deterministic.  Transformers' automatic
        # choice can vary by version/configuration, while CosyVoice needs the
        # same path for inference and for a clean diagnostic in the GUI:
        # FlashAttention 2 (when explicitly verified) -> SDPA -> eager.
        requested = os.environ.get('COSYVOICE_ATTN_IMPLEMENTATION', '').strip()
        # Do not even attempt FA2 on a CPU-only process; it would recreate the
        # very dtype/device warnings this selection is meant to prevent.
        prefer_flash = requested == 'flash_attention_2' and torch.cuda.is_available()
        self.attention_implementation = 'sdpa'

        # FlashAttention 2 requires half/bfloat16 tensors and a CUDA-resident
        # model. Loading it directly on GPU avoids Transformers' warnings and
        # prevents a CPU->GPU dispatch from silently selecting an unsafe path.
        flash_kwargs = {}
        if prefer_flash:
            device = f'cuda:{torch.cuda.current_device()}' if torch.cuda.is_available() else 'cuda:0'
            flash_kwargs = {
        ...
        try:
            ...
            if prefer_flash:
                logging.warning('FlashAttention 2 could not be initialized (%s); falling back to SDPA.', exc)
                try:
                    # Preserve the safe fp16/GPU placement when the failed
                    # FA2 backend was the only problem.
                    self.model = Qwen2ForCausalLM.from_pretrained(
                        pretrain_path,
                        sliding_window=None,
                        attn_implementation='sdpa',
                    ...
                    logging.warning('SDPA could not be initialized (%s); falling back to eager attention.', sdpa_exc)
                    self.model = Qwen2ForCausalLM.from_pretrained(
                        pretrain_path,
                        sliding_window=None,
                ...
                logging.warning('SDPA could not be initialized (%s); falling back to eager attention.', exc)
                self.model = Qwen2ForCausalLM.from_pretrained(
                    pretrain_path,
                    sliding_window=None,
                    attn_implementation='eager',
        ...
        logging.info('Qwen2 attention backend: %s', self.attention_implementation)

        # Transformers emits this once when PEFT/LayerNorm keeps an internal
        # tensor in float32 before FlashAttention casts it back to fp16.  The
        # cast is intentional and already handled by Transformers; do not
        # present it as a runtime failure in the Easy GUI console.
        if self.attention_implementation == 'flash_attention_2':
            try:
                from transformers.utils import logging as transformers_logging
                transformers_logging.get_logger('transformers.modeling_flash_attention_utils').setLevel(
                    transformers_logging.ERROR
                )
            except Exception:
                pass
...
class Qwen2LM(TransformerLM):
    ...
    def inference(
        ...
        # 5. step by step decode
        for token in self.inference_wrapper(
                lm_input, sampling, min_len, max_len, uuid,
                top_p=top_p, top_k=top_k, temperature=temperature,
                ras_window=ras_window, ras_repetition_threshold=ras_repetition_threshold):
            yield token

    @torch.inference_mode()
    def inference_wrapper(self, lm_input, sampling, min_len, max_len, uuid, top_p=0.8, top_k=25,
                          temperature=1.0, ras_window=10, ras_repetition_threshold=0.1):
        if hasattr(self, 'vllm'):
            from vllm import SamplingParams, RequestOutput
            sampling_params = SamplingParams(top_k=int(top_k),
                                             top_p=float(top_p),
