...
# These are known compatibility notices from the pinned Windows runtime, not
# actionable application failures.  Keep the filters narrow so real warnings
# (bad audio, missing files, CUDA fallback, etc.) remain visible.
warnings.filterwarnings("ignore", message=r"In 2\.9, this function's implementation will be changed.*", category=UserWarning)
warnings.filterwarnings("ignore", message=r".*torch\.cuda\.amp\.(autocast|GradScaler).*deprecated.*", category=FutureWarning)
