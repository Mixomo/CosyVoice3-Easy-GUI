...
from cosyvoice.cli.frontend import _find_local_wetext_files
...
class HybridTextNormalizer:
    ...
    def _build_wetext_normalizer(self, remove_erhua: bool | None = None) -> WeTextNormalizer:
        # Reuse the same project-local FST snapshot as the GUI.  Passing paths
        # explicitly is important: wetext otherwise calls ModelScope's
        # snapshot_download even when the files are already cached.
        wetext_root, wetext_files = _find_local_wetext_files()
        if wetext_root is not None and wetext_files is not None:
            lang = 'zh' if remove_erhua is not None else 'en'
            tagger, verbalizer = wetext_files[lang]
            return WeTextNormalizer(
                tagger_path=str(tagger),
                verbalizer_path=str(verbalizer),
                lang=lang,
                operator='tn',
                remove_erhua=bool(remove_erhua) if remove_erhua is not None else False,
            )
