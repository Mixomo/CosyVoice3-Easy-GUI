...
_CHECKPOINT_RE = re.compile(r"^resume_(epoch|step)_(\d{6})$")
            ...
            if item.is_file():
                with item.open("rb") as handle:
                    try:
                        ...
                        # the adapter was already written and closed safely.
                        pass
...
def step_checkpoint_name(completed_step: int) -> str:
    if completed_step < 0:
        raise ResumeContractError("completed_step must be non-negative")
    return f"resume_step_{completed_step:06d}"
...
def resolve_checkpoint(checkpoint: str | Path, output_dir: str | Path) -> Path:
    ...
    if not resolved.is_dir() or not _CHECKPOINT_RE.fullmatch(resolved.name):
        raise ResumeContractError(
            "Resume checkpoint must be an immutable resume_epoch_NNNNNN or resume_step_NNNNNN"
        )
    return resolved
...
def checkpoint_children(output_dir: str | Path) -> list[Path]:
    ...
    for item in output.iterdir():
        ...
        # Step progress is the authoritative chronology across both legacy epoch
        # and new step-named checkpoints; validation below reads that value.
        state_path = item / STATE_NAME
        try:
            step = int(json.loads(state_path.read_text(encoding="utf-8"))["completed_step"])
        except Exception as error:
            raise ResumeContractError(f"Checkpoint has invalid progress state: {item}") from error
        checkpoints.append((step, item.resolve(strict=True)))
    return [path for _, path in sorted(checkpoints, key=lambda pair: (pair[0], pair[1].name))]


def require_fresh_output(output_dir: str | Path) -> None:
    output = _safe_directory(output_dir)
    # The GUI creates these run-control files before starting the child. They
    # are not model state and are overwritten for each fresh launch.
    gui_scaffolding = {"run.json", "adapter_metadata.json", "training.log"}
    conflicts = [item.name for item in output.iterdir()
                 if item.name != LOCK_NAME and item.name not in gui_scaffolding]
    if conflicts:
        raise ResumeContractError(
...
def publish_checkpoint(
    *,
    ...
    output = _safe_directory(output_dir)
    name = step_checkpoint_name(completed_step) if checkpoint_unit == "step" else epoch_checkpoint_name(completed_epoch)
    target = output / name
    if target.exists() or target.is_symlink():
        raise ResumeContractError(
...
def validate_checkpoint(
    ...
    *,
    ...
    if world_size != 1 or train_engine != "single_gpu":
        raise ResumeContractError(
    ...
    expected_names = {epoch_checkpoint_name(completed_epoch), step_checkpoint_name(completed_step)}
    if selected.name not in expected_names:
        raise ResumeContractError("Checkpoint name and completed training progress disagree")
    if (
    ...
    training_config = expected_contract.get("training_config", {})
    max_steps = training_config.get("max_steps")
    max_epoch = training_config.get("max_epoch")
    if isinstance(max_steps, int) and max_steps > 0 and completed_step >= max_steps:
        raise ResumeContractError("Checkpoint already reached the configured max_steps target")
    if (not isinstance(max_steps, int) or max_steps <= 0) and isinstance(max_epoch, int) and completed_epoch + 1 >= max_epoch:
        raise ResumeContractError(
...
def _fsync_directory(path: Path) -> None:
    # Windows does not permit opening a directory as a regular fsync handle.
    # File contents are fsynced individually; directory durability is best
