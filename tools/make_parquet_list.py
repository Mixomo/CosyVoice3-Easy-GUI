...
def _init_worker(worker_utt2wav, worker_utt2text, worker_utt2spk,
                 worker_utt2embedding, worker_spk2embedding,
                 worker_utt2speech_token, worker_utt2instruct, worker_dpo_value):
    """Publish dataset maps to spawned Windows workers."""
    global utt2wav, utt2text, utt2spk, utt2embedding, spk2embedding, utt2speech_token, utt2instruct, worker_dpo
    utt2wav = worker_utt2wav
    utt2text = worker_utt2text
    utt2spk = worker_utt2spk
    utt2embedding = worker_utt2embedding
    spk2embedding = worker_spk2embedding
    utt2speech_token = worker_utt2speech_token
    utt2instruct = worker_utt2instruct
    worker_dpo = worker_dpo_value
...
if __name__ == "__main__":
    ...
    pool = multiprocessing.Pool(
        ...
        initargs=(utt2wav, utt2text, utt2spk, utt2embedding, spk2embedding, utt2speech_token, utt2instruct, args.dpo),
    )
    ...
    for i, j in enumerate(range(0, len(utts), args.num_utts_per_parquet)):
        ...
        jobs.append(pool.apply_async(job, (utts[j: j + args.num_utts_per_parquet], parquet_file, utt2parquet_file, spk2parquet_file)))
    pool.close()
    pool.join()
    # apply_async otherwise hides worker exceptions and leaves a false-success
    # data.list pointing at parquet files that were never created.
    for result in jobs:
        result.get()
